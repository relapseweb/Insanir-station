	...
	switch(blood_volume)
		if(BLOOD_VOLUME_EXCESS to BLOOD_VOLUME_MAX_LETHAL)
			if(SPT_PROB(7.5, seconds_per_tick))
				...
				inflate_gib()
			skin_tone = hexaddition(skintone2hex(original_skin_tone), "#ff0000")
			update_body(is_creating = TRUE)
		if(BLOOD_VOLUME_MAXIMUM to BLOOD_VOLUME_EXCESS)
			if(SPT_PROB(5, seconds_per_tick))
				to_chat(src, span_warning("You feel terribly bloated."))
			skin_tone = hexaddition(skintone2hex(original_skin_tone), "#770000")
			update_body(is_creating = TRUE)
		if(BLOOD_VOLUME_OKAY to BLOOD_VOLUME_SAFE)
			if(SPT_PROB(2.5, seconds_per_tick))
				to_chat(src, span_warning("You feel [word]."))
			skin_tone = original_skin_tone
			update_body(is_creating = TRUE)
		...
		if(BLOOD_VOLUME_BAD to BLOOD_VOLUME_OKAY)
			...
			skin_tone = hexaddition(skintone2hex(original_skin_tone), "#000022")
			update_body(is_creating = TRUE)
		if(BLOOD_VOLUME_SURVIVE to BLOOD_VOLUME_BAD)
			...
			skin_tone = hexaddition(skintone2hex(original_skin_tone), "#000033")
			update_body(is_creating = TRUE)
		if(-INFINITY to BLOOD_VOLUME_SURVIVE)
			if(!HAS_TRAIT(src, TRAIT_NODEATH))
				...
				death()
			to_chat(src, span_warning("You feel extremely [word]."))
			skin_tone = hexaddition(skintone2hex(original_skin_tone), "#000044")
			update_body(is_creating = TRUE)
